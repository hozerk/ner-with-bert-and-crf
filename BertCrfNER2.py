...
from transformers import BertModel, BertForTokenClassification,AdamW
...
data_dir = os.path.join(get_data_dir(), 'NER_data/CoNLL2003/').replace("\\","/")
...
output_dir = './output/'
bert_model_scale = 'dbmdz/bert-base-turkish-cased'
do_lower_case = False
# eval_batch_size = 8
...
class CoNLLDataProcessor(DataProcessor):
    ...
    def __init__(self):
        self._label_types = [ 'X', '[CLS]', '[SEP]', 'O', 'LOCATION','PERSON','ORGANIZATION']#'I-LOC', 'B-PER', 'I-PER', 'I-ORG', 'I-MISC', 'B-MISC', 'B-LOC', 'B-ORG']
    ...
    def get_train_examples(self, data_dir):
        return self._create_examples(
            self._read_data(os.path.join(data_dir, "train.txt").replace("\\","/")))

    def get_dev_examples(self, data_dir):
        return self._create_examples(
            self._read_data(os.path.join(data_dir, "valid.txt").replace("\\","/")))

    def get_test_examples(self, data_dir):
        return self._create_examples(
            self._read_data(os.path.join(data_dir, "test.txt").replace("\\","/")))
    def get_test_examples(self, data_dir):
        return self._create_examples(
            self._read_data(os.path.join(data_dir, "test.txt").replace("\\","/")))
    def get_turkish_examples_ex(self, list):
        return self._create_examples(list)
    ...
    def _create_examples_ex(self, all_lists):
        examples = []
        for (i, one_lists) in enumerate(all_lists):
            guid = i
            words = one_lists[0]
...
dfNer = pd.read_csv('C:/Users/hilal/anaconda3/envs/pythonProjectEnvDeneme/log' + '/nerCSVTriple27Jan2022.csv',encoding='utf-8')#/nerCSVTriple27Jan2022.csv', encoding='utf-8')nerCSVTriple10Nisan2022.csv
dfNer = dfNer[dfNer['sentencesUpdated'] != 'sentencesUpdated']
sentences = dfNer["sentencesUpdated"].tolist()
s=dfNer["sentences"].tolist()
ner = dfNer["ner"].tolist()
...
train_indices = arr_train[0]
test_indices = arr_test[0]
print(train_indices)
print("*******")
print(test_indices)
train_sent = []
...
for i in train_indices:
    ...
    ners= ner[i].split()
    train_ex.append([sentence, ners])
for i in test_indices:
    sentence = sentences[i].split()
    ners = ner[i].split()
    dev_ex.append([sentence, ners])

train_ex_new=conllProcessor.get_turkish_examples_ex(train_ex)
dev_ex_new=conllProcessor.get_turkish_examples_ex(dev_ex)
train_examples = train_ex_new #conllProcessor.get_train_examples(data_dir)
dev_examples = dev_ex_new #conllProcessor.get_dev_examples(data_dir)
test_examples = dev_ex_new#conllProcessor.get_test_examples(data_dir)
...
def evaluate(model, predict_dataloader, batch_size, epoch_th, dataset_name):
    ...
    with torch.no_grad():
        for batch in predict_dataloader:
            ...
            out_scores= model(input_ids=input_ids,token_type_ids= segment_ids, attention_mask=input_mask)[0]
    ...
    if valid_f1 > valid_f1_prev:
        ...
        torch.save({'epoch': epoch, 'model_state': model.state_dict(), 'valid_acc': valid_acc,
            'valid_f1': valid_f1, 'max_seq_length': max_seq_length, 'lower_case': do_lower_case},
                    os.path.join(output_dir, 'ner_bert_checkpoint.pt').replace("\\","/"))
...
#%%
if load_checkpoint and os.path.exists(output_dir+'/ner_bert_crf_checkpoint.pt'):#ner_bert_crf_checkpoint.pt
...
for epoch in range(start_epoch, total_train_epochs):
    ...
    if valid_f1 > valid_f1_prev:
        ...
        torch.save({'epoch': epoch, 'model_state': model.state_dict(), 'valid_acc': valid_acc,
            'valid_f1': valid_f1, 'max_seq_length': max_seq_length, 'lower_case': do_lower_case},
                    os.path.join(output_dir, 'ner_bert_crf_checkpoint.pt').replace("\\","/"))
        valid_f1_prev = valid_f1
